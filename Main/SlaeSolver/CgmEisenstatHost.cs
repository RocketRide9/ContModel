...
// МСГ
class CgmEisenstatHost : ISlaeSolver
{
    int _maxIter;
    Real _eps;
    ...
    public CgmEisenstatHost(int maxIter, Real eps)
    {
        _maxIter = maxIter;
    ...
    public static ISlaeSolver Construct(int maxIter, Real eps)
        => new CgmEisenstatHost(maxIter, eps);
    ...
    // https://epubs.siam.org/doi/10.1137/0902001
    public (Real discrep, int iter) SolveImpl(Types.IHalves matrix, Span<Real> b, Span<Real> x)
    {
